...
// Summit partners granted from the Agent Roster page (owner_roster.flags[email].s === 1).
// Same trust level as the Render PARTNERS list — only the owner can write owner_roster —
// it just moves the grant onto the site so John never has to touch Render for a Summit
// login. loadRoster() refreshes this from that owner-only note. Owners/deactivated are
// still resolved FIRST in roleFor, so this can only ever GRANT the summit role, never
// demote or lock anyone out.
let DYN_SUMMIT = new Set();
// Soft-deactivated logins (owner_roster.deactivated, keyed by lowercased email → when).
// A deactivated person keeps ALL their data — deals, payouts, history are untouched —
// they simply lose access. Reversible: clearing the flag restores them exactly as they
// were. Owners are never deactivatable (the owner check in roleFor runs first, and the
// admin UI + toggle endpoint both refuse an owner), so this can never lock John out.
let DEACTIVATED = new Set();
const isDeactivated = email => {
  const e = (email || "").toLowerCase();
  return DEACTIVATED.has(e) && !OWNERS.includes(e);   // owners are immune, always
};
const roleFor = email => {
  email = (email || "").toLowerCase();
  if (OWNERS.includes(email)) return "owner";
  if (isDeactivated(email)) return null;                // soft-deactivated → no access (owners already returned above)
  ...
  if (DYN_SUMMIT.has(email)) return "summit"; // Summit granted on the Agent Roster page (owner-only note)
...
if (OAUTH_READY) {
  passport.use(new GoogleStrategy({
    ...
    // Soft-deactivated login: refuse it here, and flag WHY so the callback can show the
    // "access deactivated" screen instead of the generic denied one. Checked before the
    // Premarket fallthrough so a deactivated agent can never quietly become a "member".
    if (isDeactivated(email)) return done(null, false, { deactivated: true });
...
// Live deactivation gate. Logins last 30 days, so a session minted before someone was
// deactivated would otherwise stay valid. Re-check every authenticated request against
// the current deactivated list (loadRoster is TTL-cached, so this is a fetch at most
// once per 30s, and a no-op for anonymous/asset traffic). A hit clears the session and
// sends them to the graceful screen — their data is never touched.
app.use(async (req, res, next) => {
  const email = req.user && req.user.email;
  if (!email) return next();
  const e = String(email).toLowerCase();
  if (OWNERS.includes(e)) return next();               // owner is never locked out
  try { await loadRoster(false); } catch (_) {}
  if (DEACTIVATED.has(e)) return req.logout(() => { req.session = null; res.redirect('/deactivated'); });
  next();
});
// Owner "View as" — preview the hub exactly as another person sees it. When the real
// signed-in user is an owner and they've set a viewAs target (owner-only endpoints below),
// swap req.user for that person's effective identity so EVERY downstream gate — /api/me,
// the page-access check, the per-agent data redaction — renders their view, no per-route
// changes needed. The real owner is stashed on req._owner so the set/exit endpoints can
// still confirm ownership. Google tokens stay the owner's (harmless: agent data comes from
// the shared sheet via the server-side token, not the user's Google account).
app.use((req, res, next) => {
  const va = req.session && req.session.viewAs;
  if (va && req.user && req.user.role === 'owner') {
    const e = String(va).toLowerCase().trim();
    const role = roleFor(e);                 // null if unknown/deactivated → don't impersonate
    if (role && role !== 'owner') {
      req._owner = req.user;
      req.user = { email: e, name: (req.session.viewAsName || e), role,
                   accessToken: req.user.accessToken, tokenExp: req.user.tokenExp, viewing: true };
    }
  }
  next();
});
...
// HQ login-page hero — its OWN file so it never collides with the Reserve hero.mp4 above.
// Public (the login page is shown signed-out), served straight from the repo root.
app.get("/hqhero.mp4", (req, res) =>
  res.sendFile(path.join(__dirname, "hqhero.mp4"), err => { if (err && !res.headersSent) res.status(404).end(); }));
...
// Custom callback so a soft-deactivated login lands on the "access deactivated" screen
// (info.deactivated set in the verify), while an unknown account still gets /denied.
app.get("/auth/google/callback", (req, res, next) => {
  passport.authenticate("google", (err, user, info) => {
    if (err) return next(err);
    if (!user) return res.redirect(info && info.deactivated ? "/deactivated" : "/denied");
    req.logIn(user, e => {
      if (e) return next(e);
      ...
      res.redirect(user.role === "member" ? "/premarket-hub.html" : "/");
    });
  })(req, res, next);
});
...
// Graceful full-screen notice for a deactivated login. Reassures them their records are
// still on file (nothing was deleted) and points them to the owner. Public route (they're
// signed out by the time they see it) so it renders without a login loop.
app.get("/deactivated", (req, res) =>
  res.status(403).type('html').send(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Access deactivated</title>
<style>*{box-sizing:border-box;margin:0}html,body{height:100%}body{display:flex;align-items:center;justify-content:center;background:#0B0C0E;color:#fff;font-family:Inter,system-ui,-apple-system,sans-serif;padding:24px;text-align:center}.box{max-width:440px}.mk{width:52px;height:52px;border-radius:50%;border:2px solid #E11D2E;color:#E11D2E;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:800;margin:0 auto 20px}h1{font-size:22px;font-weight:800;letter-spacing:-.01em;margin-bottom:10px}p{font-size:15px;line-height:1.55;color:#c3ccdb}.sub{margin-top:14px;font-size:13px;color:#8b93a3}</style></head>
<body><div class="box"><div class="mk">&#10005;</div><h1>Access deactivated</h1><p>Your Grove HQ access has been turned off. <b style="color:#e7ebf2">Your records are still on file</b> &mdash; nothing has been deleted.</p><p class="sub">If you think this is a mistake, contact the owner to have your access restored.</p></div></body></html>`));
// Owner-only: start / stop "View as". Ownership is checked against the REAL user
// (req._owner when already impersonating, otherwise req.user), so an impersonated
// session can always exit but can never re-target without being a real owner.
app.get('/api/viewas', async (req, res) => {
  const real = req._owner || req.user;
  if (!real || real.role !== 'owner') return res.status(403).json({ ok: false, error: 'forbidden' });
  const e = String(req.query.email || '').toLowerCase().trim();
  try { await loadRoster(true); } catch (_) {}   // pick up a just-added person immediately
  const role = roleFor(e);
  if (!e || !role || role === 'owner') return res.status(400).json({ ok: false, error: 'This login can’t be previewed (unknown, deactivated, or another owner).' });
  req.session.viewAs = e;
  req.session.viewAsName = String(req.query.name || '').slice(0, 80) || e;
  res.json({ ok: true, viewing: e, role });
});
app.get('/api/viewas/exit', (req, res) => {
  if (req.session) { delete req.session.viewAs; delete req.session.viewAsName; }
  res.json({ ok: true });
});
...
// --- Pexels royalty-free photo search (key stays server-side; never shipped to client) ---
let _pxCache = {}; // query -> {at,data}
app.get("/api/pexels", ensureAuth, async (req, res) => {
  try {
    const key = process.env.PEXELS_API_KEY || "";
    if (!key) return res.json({ ok: false, error: "no_key", photos: [] });
    const q = String(req.query.q || "").trim();
    const page = Math.max(1, parseInt(req.query.page, 10) || 1);
    const per = Math.min(40, Math.max(1, parseInt(req.query.per, 10) || 24));
    const orient = String(req.query.orientation || "").trim(); // landscape|portrait|square
    const ck = q.toLowerCase() + "|" + page + "|" + per + "|" + orient;
    const hit = _pxCache[ck];
    if (hit && Date.now() - hit.at < 30 * 60 * 1000) return res.json(hit.data);
    let url;
    if (q) {
      url = "https://api.pexels.com/v1/search?query=" + encodeURIComponent(q) +
            "&per_page=" + per + "&page=" + page + (orient ? "&orientation=" + encodeURIComponent(orient) : "");
    } else {
      url = "https://api.pexels.com/v1/curated?per_page=" + per + "&page=" + page;
    }
    const r = await fetch(url, { headers: { Authorization: key } });
    if (!r.ok) return res.status(502).json({ ok: false, error: "pexels_" + r.status, photos: [] });
    ...
    const data = { ok: true, page, per_page: per, total: j.total_results || photos.length,
                   next: !!j.next_page, query: q, photos, source: "Pexels" };
    _pxCache[ck] = { at: Date.now(), data };
    res.json(data);
  } catch (e) { res.status(502).json({ ok: false, error: "pexels_failed", photos: [] }); }
});

// Same-origin image proxy for Pexels CDN — lets the builder pull a photo into a
// data URL without tainting the export canvas. Only images.pexels.com is allowed.
app.get("/api/pexels/img", ensureAuth, async (req, res) => {
  try {
    const u = String(req.query.u || "");
    let host = "";
    try { host = new URL(u).hostname.toLowerCase(); } catch (e) { return res.status(400).end(); }
    if (!(host === "images.pexels.com" || host.endsWith(".pexels.com"))) return res.status(400).end();
...
// Push a lead INTO REsimpli. REsimpli has no public lead-create API, so it can
// only be reached through Zapier. TWO delivery routes are supported:
//
//   RESIMPLI_QUEUE_URL (preferred) — a tiny Google Apps Script Web App bound to
//     a "REsimpli Queue" spreadsheet. The button appends one row per lead, and
//     a Zapier "Google Sheets → REsimpli: Create Lead" Zap picks it up. This
//     avoids "Webhooks by Zapier", which is a PAID Zapier feature — a Google
//     Sheets trigger runs on far cheaper tiers, and the sheet doubles as a
//     visible log of everything ever pushed.
//   RESIMPLI_ZAP_URL (fallback) — the old "Webhooks by Zapier" catch URL, kept
//     so nothing breaks for anyone still on the webhook Zap.
//
// John + Summit only.
const RESIMPLI_QUEUE_URL   = process.env.RESIMPLI_QUEUE_URL || '';
const RESIMPLI_QUEUE_TOKEN = process.env.RESIMPLI_QUEUE_TOKEN || 'grove-sheet-2026';
...
app.post('/api/resimpli/push', ensureAuth, async (req, res) => {
  if (!(req.user.role === 'owner' || req.user.role === 'summit'))
    return res.status(403).json({ ok: false, error: 'forbidden' });
  if (!RESIMPLI_QUEUE_URL && !RESIMPLI_ZAP_URL) return res.status(503).json({ ok: false, error: 'not_configured', message: 'REsimpli isn\'t connected yet — set RESIMPLI_QUEUE_URL (the Google Sheet queue) in the server.' });
  const b = req.body || {};
  // REsimpli's "Create Lead" makes Phone a REQUIRED field, but most probate
  // filings have no phone until they're skip-traced. Sending a blank makes
  // REsimpli reject the whole lead. So when there's no real number, send a
  // recognizable placeholder — 000-000-0000 — which passes the required check
  // AND tells you at a glance in REsimpli that this lead still needs a number.
  const rawPhone = String(b.phone || '').replace(/[^0-9]/g, '');
  const phone = rawPhone.length >= 7 ? String(b.phone).slice(0, 40) : '000-000-0000';
  const lead = {
    name: String(b.name || '').slice(0, 120),
    address: String(b.address || '').slice(0, 200),
    phone: phone,
    phone_missing: rawPhone.length >= 7 ? '' : 'yes',   // Zapier can branch/tag on this if you ever want
    email: String(b.email || '').slice(0, 120),
    source: String(b.source || 'GPG Hub').slice(0, 80),
    notes: String(b.notes || '').slice(0, 800) + (rawPhone.length >= 7 ? '' : ' · NO PHONE ON FILE — skip-trace needed'),
  ...
  try {
    if (RESIMPLI_QUEUE_URL) {
      // Append a row to the Google Sheet queue. The Apps Script returns
      // {ok:true} on success; a non-2xx or {ok:false} means the row didn't land.
      const r = await fetch(RESIMPLI_QUEUE_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(Object.assign({ token: RESIMPLI_QUEUE_TOKEN }, lead))
      });
      const txt = await r.text();
      if (!r.ok) throw new Error('queue_http_' + r.status);
      if (txt.replace(/\s/g, '').indexOf('"ok":true') < 0) throw new Error('queue_rejected: ' + txt.slice(0, 120));
      return res.json({ ok: true, via: 'sheet' });
    }
...
app.get("/api/lead-list", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    byId.forEach((L, id) => {
      ...
      // On a MA probate petition the petitioner IS the proposed Personal Representative,
      // so surface the PDF-read name + address on the "Personal Rep" rows the tile leads
      // with (the wholesaler's actual contact), not just as "Petitioner".
      addF('Personal Rep', x.petitioner_name);
      addF('PR address', x.petitioner_address);
...
const GROVE_ACCESS =
   ...
   ':root{--paper:#F2F3F6;--card:#FFFFFF;--card2:#F5F7FA;--panel:#F5F7FA;--sand:#F5F7FA;--line:#E8EAEE;--line2:#DFE2E8;--fieldline:#DFE2E8;--ink:#0B0C0E;--slate:#0B0C0E;--mut:#5B6472;--mut2:#8A93A3;--blue:#0064E5;--blued:#0056C9;--bluebg:#EAF1FE;--accent:#0064E5;--accent-hover:#0056C9;--accdim:#EAF1FE;--brand:#0064E5;--brandmut:#9DC0F5;--navy:#0B0C0E;--nav:#0064E5;--black:#0064E5;--headerbg:#FFFFFF;--green:#1E7A52;--good:#1E7A52;--goodbg:#E7F4EC;--red:#E11D2E;--bad:#E11D2E;--badbg:#FDE8EA;--amber:#5B6472;--warn:#5B6472;--warnbg:#F3F5F8;--teal:#166534;--coral:#6D28D9;--sky:#4C8DFF}'
   /* dark tokens */
 + 'html.dark{--paper:#0E1116;--card:#1B2027;--card2:#232A33;--panel:#232A33;--sand:#232A33;--line:#2A303C;--line2:#3A4150;--fieldline:#3A4150;--ink:#DDE1E8;--slate:#DDE1E8;--mut:#8A92A0;--mut2:#8C95A3;--blue:#4C8DFF;--blued:#6AA1FF;--bluebg:#12233F;--accent:#4C8DFF;--accent-hover:#6AA1FF;--accdim:#12233F;--brand:#0F1319;--brandmut:#3A4150;--navy:#0C0F14;--nav:#0F1319;--black:#0F1319;--headerbg:#181D25;--green:#3FBE86;--good:#3FBE86;--goodbg:#14352A;--red:#FF6B6B;--bad:#FF6B6B;--badbg:#3A1E16;--amber:#9AA1AD;--warn:#9AA1AD;--warnbg:#232A33;--teal:#3F9E6E;--coral:#A78BFA;--sky:#7FA9F5}'
 ...
 + 'h1,h2,h3,h4,.hublogo,.ttl,.brandline,.crumb,.tbar .crumb{font-family:Inter,system-ui,-apple-system,sans-serif!important;font-weight:800!important;letter-spacing:-.02em}'
 ...
 + 'header .sub,header .tsub,header p,header .tagline,header .mini{color:var(--mut)!important}'
   /* Uniform page bar: many tool pages use <div class="top"> with a solid blue fill,
      while others use a white <header>. Normalize .top to the same surface header so
      every page shows the same bar (light in light mode, dark header in dark mode). */
 + '.top{background:var(--headerbg)!important;color:var(--ink)!important;border-bottom:1px solid var(--line)!important;box-shadow:none!important}'
 + '.top h1,.top h2,.top .ttl,.top b,.top .hublogo{color:var(--ink)!important}'
 + '.top a.hq,.top .hq{color:var(--accent)!important;font-weight:600!important}'
 + '.top .sync,.top .sub,.top .mini,.top .tsub,.top small{color:var(--mut)!important}'
 ...
 + 'header .hbtn,header .btn:not(.primary),header button:not(.primary):not(.cotab){background:var(--card2)!important;color:var(--ink)!important;border:1px solid var(--line)!important}'
   /* Company tabs (home shell) own their own active/brand styling — never let the header
      button normalizer flatten the selected tab to a light chip. */
 + '.cotab.on{color:#fff!important;background:var(--tc,#17418A)!important;border-color:var(--tc,#17418A)!important}'
 + '.cotab.on .cm{background:rgba(255,255,255,.22)!important}'
 ...
 + '.seg button.active,.seg button.on{background:var(--blue)!important;color:#fff!important;border-color:var(--blue)!important}'
   /* dark-mode legibility fixes: navy-colored numbers were near-black on a near-black
      card (invisible); make any inline navy text light. Strengthen tile/card borders,
      and stop the pipeline chips + tiles from rendering on a stray white fallback. */
 + 'html.dark [style^="color:var(--navy)"],html.dark [style*=";color:var(--navy)"]{color:#D8DFEC!important}'
 + 'html.dark .tot .s,html.dark .tile,html.dark .stat,html.dark .snap,html.dark .pchip,html.dark .kpi{border-color:var(--line2)!important}'
 + 'html.dark .pchip{background:var(--card)!important}html.dark .pchip.folder{background:var(--card2)!important}'
 + 'html.dark .pchip .pcct{background:#4A5364!important}html.dark .pchip.on .pcct{background:var(--blue)!important}'
 + 'html.dark .flag{background:var(--card2)!important;border-color:var(--line2)!important;color:var(--ink)!important}'
 + 'html.dark .mstrip,html.dark .mstrip .m{border-color:var(--line2)!important}'
   /* Dark-mode audit (2026-08-21): make FIELDS visibly distinct from the page/card,
      and rescue hardcoded near-black / dark-navy inline text that vanished on dark. */
 + 'html.dark input:not([type=checkbox]):not([type=radio]):not([type=range]):not([type=color]):not([type=button]):not([type=submit]):not([type=file]),html.dark select,html.dark textarea{background:#262E39!important;border:1px solid #4A5464!important;color:#EAEEF5!important}'
 + 'html.dark input::placeholder,html.dark textarea::placeholder{color:#838C99!important;opacity:1}'
 + 'html.dark input:focus,html.dark select:focus,html.dark textarea:focus{border-color:var(--blue)!important;box-shadow:0 0 0 2px rgba(76,141,255,.28)!important;outline:none!important}'
 + 'html.dark [style*="color:#0a0a0b"],html.dark [style*="color:#0A0A0B"],html.dark [style*="color:#0a0a0a"],html.dark [style*="color:#14140f"],html.dark [style*="color:#14140F"],html.dark [style*="color:#0b1020"],html.dark [style*="color:#0B1020"],html.dark [style*="color:#1c2233"],html.dark [style*="color:#1C2233"],html.dark [style*="color:#141414"],html.dark [style*="color:#141a24"],html.dark [style*="color:#141A24"],html.dark [style*="color:#0B0B0B"],html.dark [style*="color:#000000"],html.dark [style*="color:#000;"],html.dark [style*="color:#111;"],html.dark [style*="color:#111827"]{color:var(--ink)!important}'
 + 'html.dark [style*="color:#17418a"],html.dark [style*="color:#17418A"],html.dark [style*="color:#0B1020"]{color:#79A9FF!important}'
 + 'html.dark [style*="color:#334155"],html.dark [style*="color:#1f2937"],html.dark [style*="color:#1F2937"]{color:#B3BCCB!important}'
 + 'html.dark [style*="background:#fff"],html.dark [style*="background:#ffffff"],html.dark [style*="background-color:#fff"],html.dark [style*="background:#FFF"],html.dark [style*="background:white"]{background:var(--card)!important}'
   /* navy-as-TEXT rescue: names, headings, tile numbers etc. colour their text with
      var(--navy), which the dark theme turns near-black. Lighten those specific
      plain-text selectors (buttons / active chips deliberately excluded). */
 + 'html.dark .bcard .bmeta,html.dark .card h2,html.dark .cbprev tr.gv td,html.dark .cl-pct,html.dark .conf-body p.caps,html.dark .ct a,html.dark .deal-item .di .t,html.dark .dl td:last-child,html.dark .doc-head .prop,html.dark .doc-secrow .pg,html.dark .doc-secrow .sec,html.dark .f-sec>h3,html.dark .grptop .gt,html.dark .mini-tot b,html.dark .modal h2,html.dark .mstrip .n,html.dark .no-goals-prompt .set-link,html.dark .pname,html.dark .prose .h,html.dark .roster th,html.dark .rt th,html.dark .sec>h3,html.dark .sechead,html.dark .side .sh b,html.dark .stat b,html.dark .stat-card .val,html.dark .tile b,html.dark .toc-row .pg,html.dark .toc-row .t,html.dark .tot .s span,html.dark .total .tv,html.dark .tpl .t,html.dark .who b,html.dark .who b a,html.dark h1,html.dark h2,html.dark h3,html.dark th{color:var(--ink)!important}'
   /* Polish pass: many tool pages redefine --accent/--blue/--bad in their own :root
      back to the old navy/brick. Force the core accent + error tokens with !important
      (custom props with !important win regardless of source order) so every page's
      buttons, links, active states and errors follow the AXESS palette. Semantic
      greens/warn/teal/coral and Grove-navy logos are left untouched. */
 + ':root{--accent:#0064E5!important;--accent-hover:#0056C9!important;--blue:#0064E5!important;--blued:#0056C9!important;--bluebg:#EAF1FE!important;--accdim:#EAF1FE!important;--accent-weak:#EAF1FE!important;--bad:#E11D2E!important;--red:#E11D2E!important;--badbg:#FDE8EA!important}'
 + 'html.dark{--accent:#4C8DFF!important;--accent-hover:#6AA1FF!important;--blue:#4C8DFF!important;--blued:#6AA1FF!important;--bluebg:#12233F!important;--accdim:#12233F!important;--accent-weak:#12233F!important;--bad:#FF6B6B!important;--red:#FF6B6B!important;--badbg:#3A1E16!important}'
...
const SUMMIT_TOOL_NOTE = /^(offers|investors|rehabs|track_record|_heartbeat|uw_store_summit|lead_status|om_deals|todos_personal|summit_logins|summit_deals|summit_props|summit_voicenotes|summit_tasks|summit_network)$/i;
// Summit networking directory logos live one-per-contact as network_logo_<id>, mirroring
// the Grove vendor-logo pattern; Summit + owner may read and write them.
const SUMMIT_NOTE_READ_OK  = k => { k = String(k || '').toLowerCase(); return k === 'hub_access' || /^social_/.test(k) || /^network_logo_/.test(k) || SUMMIT_TOOL_NOTE.test(k); };
const SUMMIT_NOTE_WRITE_OK = k => { k = String(k || '').toLowerCase(); return (/^social_/.test(k) && k !== 'social_brand_access') || /^network_logo_/.test(k) || SUMMIT_TOOL_NOTE.test(k); };
...
const CITICORE_NOTE_OK = k => { k = String(k || '').toLowerCase(); return k === 'owner_portfolio' || k === 'owner_rentroll' || k === 'owner_loans' || /^condofees_/.test(k); };
function ownerOnlyNote(action, notekey, role, write, email){
  ...
  // AXESS Rolodex: John (owner, already allowed above) + Angelica only — read + write.
  if (/^axess_rolodex$/i.test(String(notekey || ''))) {
    return String(email || '').toLowerCase() !== 'rentals@gpgboston.com';
  }
...
const AGENT_NOTE_OK = k => {
  ...
  // pm_* raw notes intentionally NOT allowed here — the marketplace is reached only
  // through /api/pm/* (server-side address redaction + per-record ownership). Raw note
  // access would hand out every off-market address and let anyone overwrite listings.
  k = String(k || '').toLowerCase();
  if (k === '_heartbeat' || k === 'hub_access') return true;
  if (/^(todos_personal|personal_note|om_deals|tx_checklists|todos_shared|grove_leads|quick_links|forsale_folder|grove_vendors)$/.test(k)) return true;
  if (/^(todos_|td_|ql_|quicklinks_|vendor_logo_)/.test(k)) return true;   // shared vendor directory + per-vendor logos
  return false;
};
...
function addMag(el){if(el.__gmag)return;el.__gmag=1;var host=el.parentNode;if(!host)return;
// Pin the lookup 🔍 INSIDE the input at the right edge instead of dropping a
// bordered button on the next line (full-width inputs made it wrap and look broken).
var wrap=document.createElement('span');wrap.className='__gmagwrap';wrap.style.cssText='position:relative;display:block';
try{host.insertBefore(wrap,el);wrap.appendChild(el);}catch(e){el.__gmag=0;return;}
try{var pr=parseInt(el.style.paddingRight||'0',10);if(!pr||pr<30)el.style.paddingRight='34px';}catch(e){}
var b=document.createElement('button');b.type='button';b.textContent='🔍';b.title='Owner lookup & research — opens in a new tab';
b.style.cssText='position:absolute;right:8px;top:50%;transform:translateY(-50%);border:0;background:transparent;cursor:pointer;font-size:14px;line-height:1;padding:0;opacity:.55;z-index:2';
b.addEventListener('mouseenter',function(){b.style.opacity='1';});b.addEventListener('mouseleave',function(){b.style.opacity='.55';});
b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();var q=(el.value||'').trim();window.open('/owner-lookup.html?q='+encodeURIComponent(q),'_blank','noopener');};
wrap.appendChild(b);}
...
// Close the assessed-value lookup box on any click outside it (never on the address input itself).
document.addEventListener('mousedown',function(e){var t=e.target;if(box&&t&&!box.contains(t)&&!(t&&t.__gprop))closeBox();},true);
// Also close it when focus leaves the page (e.g. clicking into a tool iframe — that
// swallows the outside-click, which is what left it "stuck" over the sidebar) and on
// any route change. Belt-and-suspenders so a stray dropdown never lingers.
window.addEventListener('blur',closeBox);
window.addEventListener('hashchange',closeBox);
document.addEventListener('visibilitychange',function(){if(document.hidden)closeBox();});
...
app.get("/", (req, res) => {
  ...
  // Signed-out visitors get the public welcome page (Boston hero + "Continue with
  // Google"). Its button points at /auth/google, which signs them in and lands them
  // right back here — now authenticated — so the hub renders below. This replaces the
  // old behaviour where "/" bounced straight to Google's account chooser with no
  // branded front door in between.
  if (!req.isAuthenticated()) {
    return res.sendFile(path.join(__dirname, "landing.html"), err => {
      if (err && !res.headersSent) res.redirect("/auth/google");   // landing missing → old flow
    });
  }
  ...
  const inject = `<script>window.__ROLE__=${JSON.stringify(req.user.role)};window.__USER__=${JSON.stringify(req.user.name)};window.__EMAIL__=${JSON.stringify(req.user.email)};window.__AGENTNAME__=${JSON.stringify(agentDealName(req.user.email))};window.__ACCT__=${JSON.stringify(acctFor(req.user.role))};window.__MAPSKEY__=${JSON.stringify(GOOGLE_MAPS_KEY)};window.__VIEWING__=${JSON.stringify(!!req.user.viewing)};</script>`;
  // When the owner is previewing "as" someone, a red bar rides the top of the hub so
  // it's unmistakable, with a one-tap Exit that clears the preview and reloads as owner.
  const viewBar = req.user.viewing ? `<script>document.addEventListener('DOMContentLoaded',function(){var lbl=${JSON.stringify('Previewing as ' + (req.user.name || req.user.email) + ' (' + req.user.role + ')')};var b=document.createElement('div');b.style.cssText='position:fixed;top:0;left:0;right:0;z-index:2147483600;background:#E11D2E;color:#fff;font:600 13px Inter,system-ui,sans-serif;padding:8px 14px;display:flex;align-items:center;justify-content:center;gap:14px;box-shadow:0 2px 10px rgba(0,0,0,.25)';b.innerHTML='\\uD83D\\uDC41\\uFE0F '+lbl+' <button style="background:#fff;color:#E11D2E;border:0;border-radius:7px;padding:4px 12px;font:inherit;font-weight:800;cursor:pointer">Exit preview</button>';b.querySelector('button').onclick=function(){fetch('/api/viewas/exit',{credentials:'include'}).then(function(){location.href='/';});};document.body.appendChild(b);document.body.style.paddingTop='36px';});</script>` : '';
  html = html.replace("</head>", inject + viewBar + "</head>");
...
app.get("/api/grove", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    // For-Sale Folder: listings can be public (share==null), restricted to a set of
    // emails (share=[...]), or private to their creator. for-sale-folder.html's canSee()
    // did this filtering in the browser only, so the note shipped every "Only you"
    // listing to every agent. Mirror canSee() server-side: an agent receives public
    // rows, rows they created (by === their name), and rows explicitly shared to their
    // email. Owner is untouched (full inventory + share chips).
    if (action === 'notes' && String(req.query.notekey || '').toLowerCase() === 'forsale_folder'
        && req.user.role !== 'owner' && j) {
      const myName  = String(req.user.name || '').trim();
      const myEmail = String(req.user.email || '').toLowerCase().trim();
      const canSee  = p => {
        if (!p || p.share == null) return true;
        if (myName && String(p.by || '').trim() === myName) return true;
        return (Array.isArray(p.share) ? p.share : []).some(e => String(e || '').toLowerCase().trim() === myEmail);
      };
      const raw = (j.value != null ? j.value : j.notes);
      let out = raw;
      try {
        const obj = JSON.parse(String(raw || '{}')) || {};
        const arr = Array.isArray(obj.props) ? obj.props : [];
        obj.props = arr.filter(canSee);
        out = JSON.stringify(obj);
      } catch (e) { out = JSON.stringify({ props: [] }); }  // unreadable → send nothing
      return res.json(Object.assign({}, j, { value: out, notes: out }));
    }
    // Password vaults (Grove + Summit): the page hid passwords behind an unlock code,
    // but the note shipped every cleartext password to anyone allowed to read it — the
    // code only decided whether the browser painted them. Gate it at the source: the
    // client re-fetches with ?unlock=<code hash> after the code is entered, and the
    // server returns passwords only when that proof matches the stored *_pin note (or
    // no code is set). Without a valid proof every `pass` field ships blank, so the
    // secrets never leave the server until the code is proven. Other fields (site, URL,
    // username, notes) are unchanged so the list still loads.
    {
      const vk = String(req.query.notekey || '').toLowerCase();
      if (action === 'notes' && (vk === 'grove_logins' || vk === 'summit_logins') && j) {
        let pinHash = '';
        try {
          const pu = GROVE_SHEET_URL + sheetSep(GROVE_SHEET_URL) + 'token=' + encodeURIComponent(GROVE_SHEET_TOKEN) + '&action=notes&notekey=' + vk + '_pin';
          const pj = await fetch(pu).then(r => r.json());
          pinHash = String((pj && (pj.value != null ? pj.value : pj.notes)) || '').trim();
        } catch (e) { pinHash = ''; }
        const provided = String(req.query.unlock || '').trim();
        const unlocked = !pinHash || provided === pinHash;   // no code set → open, as the page already treats it
        if (!unlocked) {
          const raw = (j.value != null ? j.value : j.notes);
          let out = '[]';
          try {
            const rows = JSON.parse(String(raw || '[]'));
            out = JSON.stringify((Array.isArray(rows) ? rows : []).map(r => Object.assign({}, r, { pass: '' })));
          } catch (e) { out = '[]'; }
          return res.json(Object.assign({}, j, { value: out, notes: out }));
        }
      }
...
app.post("/api/grove", ensureAuth, async (req, res) => {
  ...
  if (req.user.role === 'summit' && action !== 'notes')
    return res.status(403).json({ ok: false, error: 'forbidden' });
  // Deal-ledger + deposit writes are for Grove agents (and the owner) only — a Citicore
  // guest, a bookkeeper, or a public marketplace member must not be able to inject a
  // fabricated commission row or trigger the new-deal email.
  if ((action === 'agentDeal' || action === 'depositSave') && !(req.user.role === 'agent' || req.user.role === 'owner'))
    return res.status(403).json({ ok: false, error: 'forbidden' });
  ...
  // Password-vault write guard. Reads now blank every `pass` until the unlock code is
  // proven, so a still-locked browser holds rows with empty passwords. If it saves
  // (editing a URL, adding a row) that blank would overwrite the real password on the
  // sheet. Before writing grove_logins/summit_logins, re-read the stored rows and, for
  // any incoming row whose `pass` is blank, restore the stored password by id. A
  // deliberately cleared password (unlocked user empties the field) still saves,
  // because an unlocked client always carries the real values it just fetched.
  {
    const wvk = String(b.notekey || '').toLowerCase();
    if (action === 'notes' && (wvk === 'grove_logins' || wvk === 'summit_logins')) {
      try {
        const incoming = JSON.parse(String(b.value != null ? b.value : (b.notes != null ? b.notes : '[]')) || '[]');
        if (Array.isArray(incoming) && incoming.some(r => r && (r.pass == null || r.pass === ''))) {
          const pu = GROVE_SHEET_URL + sheetSep(GROVE_SHEET_URL) + 'token=' + encodeURIComponent(GROVE_SHEET_TOKEN) + '&action=notes&notekey=' + wvk;
          const pj = await fetch(pu).then(r => r.json());
          const praw = pj && (pj.value != null ? pj.value : pj.notes);
          const prev = JSON.parse(String(praw || '[]'));
          const byId = {};
          (Array.isArray(prev) ? prev : []).forEach(r => { if (r && r.id != null) byId[r.id] = r; });
          const merged = incoming.map(r => {
            if (r && (r.pass == null || r.pass === '') && r.id != null && byId[r.id] && byId[r.id].pass)
              return Object.assign({}, r, { pass: byId[r.id].pass });
            return r;
          });
          const mstr = JSON.stringify(merged);
          if (b.value != null) b.value = mstr; else b.notes = mstr;
        }
      } catch (e) { /* unreadable prior/incoming → write as-is, never block a save */ }
    }
  }
  ...
  try {
    ...
    deals.forEach(function(d) {
      ...
      out.push({
        ...
        price: (d.price != null ? d.price : ''),   // the sale price (public) — NOT a fee/split; used for volume + address columns
...
// Public marketing stats for the landing ticker + the hub dashboard AXESS tile —
// aggregate only, no auth, no addresses.
//
// The AXESS marketplace has spun out to its own deploy with its OWN Google Sheet;
// the hub's legacy pm_* notes are stale (they still hold pre-launch demo listings).
// So the source of truth is the live AXESS app: proxy its public stats here (a
// server-to-server call, so no CORS), and fall back to the hub's own note store
// only if the AXESS app is unreachable. Override the URL with AXESS_STATS_URL.
const AXESS_STATS_URL = process.env.AXESS_STATS_URL || 'https://access-network.onrender.com/api/pm/stats';
app.get('/api/pm/stats', async (req, res) => {
  // 1) Prefer the live AXESS app.
  try {
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), 4000);
    const r = await fetch(AXESS_STATS_URL, { signal: ctl.signal, headers: { accept: 'application/json' } });
    clearTimeout(timer);
    if (r.ok) {
      const j = await r.json();
      if (j && j.ok && j.stats) return res.json({ ok: true, stats: j.stats, source: 'axess' });
    }
  } catch (e) { /* AXESS app unreachable — fall back to local note store below */ }
  // 2) Fallback: compute from the hub's own note store (legacy).
  try {
...
const ACL_HIDE = new Set(['home.html','hub-v3.html','access-control.html','privacy.html','terms.html',
                          ...
                          // AXESS Rolodex — access is fixed to owner + Angelica by email, not the grid.
                          'axess-rolodex.html']);
...
const ACL_SENSITIVE = new Set(['money.html','grove-books.html','deals-payouts.html','deals-payouts2.html','payouts.html','logins.html',
...
const OWNER_ONLY_PAGES = new Set([
  ...
  'qbo-connect.html','qbo-connect-summit.html','deals-payouts.html','deals-payouts2.html','payouts.html','money.html',
  'key-dates.html',
...
const SUMMIT_PAGES = new Set([
  ...
  'summit-network.html',      // Summit networking directory (banks, attorneys, buyers, sellers…)
...
async function loadRoster(force){
  ...
  try {
    ...
    // Summit grants: any person whose roster flag `s` is on (the "Summit" checkbox
    // on the Agent Roster page). Owner-only note, so this is a trusted grant.
    const sumset = new Set();
    const flags = (obj && obj.flags && typeof obj.flags === 'object') ? obj.flags : {};
    Object.keys(flags).forEach(k => {
      const e = String(k || '').trim().toLowerCase();
      const f = flags[k];
      if (e.indexOf('@') > 0 && f && (f.s === 1 || f.s === true)) sumset.add(e);
    });
    // Deactivated logins live on the same owner-only note as { email: whenISO, ... }.
    // Accept either the object form or a plain array of emails, so a hand-edited note
    // still works. Never delete the person from custom/flags — this only revokes access.
    const dset = new Set();
    const draw = obj && obj.deactivated;
    if (draw && typeof draw === 'object' && !Array.isArray(draw)) {
      Object.keys(draw).forEach(k => { const e = String(k || '').trim().toLowerCase(); if (e.indexOf('@') > 0) dset.add(e); });
    } else if (Array.isArray(draw)) {
      draw.forEach(x => { const e = String((x && x.email) || x || '').trim().toLowerCase(); if (e.indexOf('@') > 0) dset.add(e); });
    }
    DYN_AGENTS = set; DYN_AGENT_NAMES = names; DYN_SUMMIT = sumset; DEACTIVATED = dset; ROSTER_AT = Date.now();
  } catch (e) {
...
function pageBlockedFor(pg, role, email){
  ...
  // AXESS Rolodex: owner (above) + Angelica only. Not a Summit-wide page.
  if (pg === 'axess-rolodex.html') return String(email || '').toLowerCase() !== 'rentals@gpgboston.com';
